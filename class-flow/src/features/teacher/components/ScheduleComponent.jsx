...
import { getTeacherSchedule } from '../service/teacherService';
...
// Main App component
function ScheduleTableComponent({teacherId, setTableData, filters, scheduleId }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            if (setTableData) setTableData(classes);
                                ...
                                if (specialEvent) {
                                    return (
                                        <TableRow key={time}>
                                            ...
                                            <TableCell colSpan={allSectionsCount} className="text-center font-bold text-lg text-dark dark:text-white bg-muted">
                                                {specialEvent.title}
                                            </TableCell>
                                        </TableRow>
                                    );
                                                    ...
                                                    {classesList.length > 0 ? (
                                                        <div className="flex flex-col space-y-2">
                                                            {classesList.map(cls => {
                                                                ...
                                                                return (
                                                                    <div key={`${cls.subject_code}-${cls.teacher_name}`} className={`p-2 rounded-lg ${bg} ${text} shadow-sm transition-transform transform hover:scale-105`}>
