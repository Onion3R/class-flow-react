...
function ScheduleTableComponent({ scheduleId }) {
    ...
    useEffect(() => {
        ...
        useEffect(() => {
     console.log('grade11:', grade11Data);
    }, [grade11Data])
    ...
    return (
        <div className="flex flex-col items-center w-full  mx-auto  mt-4">
            <div className="rounded-xl overflow-hidden w-full border border-gray-200 ">
                <div className="overflow-x-auto">
                    <Table className="min-w-full ">
                        <TableHeader>
                            {/* Grade Level headers row */}
                            <TableRow>
                                <TableHead rowSpan={3} className="text-center font-semibold border-r  border-gray-200 ">Time</TableHead>
                                {grade11SectionsCount > 0 && (
                                    <TableHead colSpan={grade11SectionsCount} className="text-center border-b border-gray-200 font-bold text-lg  ">
                                        Grade 11
                                    </TableHead>
                                )}
                                {grade12SectionsCount > 0 && (
                                    <TableHead colSpan={grade12SectionsCount} className="text-center border-b border-gray-200 font-bold text-lg  ">
                                        Grade 12
                                    </TableHead>
                                )}
                            </TableRow>
                            ...
                            <TableRow>
                                ...
                                {grade11Data && Object.keys(grade11Data.strandSections).map(strandName => (
                                    <TableHead key={`g11-strand-${strandName}`} colSpan={grade11Data.strandSections[strandName].length} className="text-center border-l border-gray-200 font-semibold text-base border-b border-r">{strandName}</TableHead>
                                ))}
                                {/* Grade 12 Strands */}
                                {grade12Data && Object.keys(grade12Data.strandSections).map(strandName => (
                                    <TableHead key={`g12-strand-${strandName}`} colSpan={grade12Data.strandSections[strandName].length} className="text-center border-l border-gray-200 font-semibold text-base border-b border-r">{strandName}</TableHead>
                                ))}
                            </TableRow>
                            {/* Section headers row */}
                            <TableRow>
                        ...
                        <TableBody>
                            {combinedTimeSlots.map(time => {
                                ...
                                if (specialEvent) {
                                    return (
                                        <TableRow key={time}>
                                            <TableCell className="font-medium text-center border-r border-gray-300 min-w-[70px]  ">
                                            ...
                                            <TableCell colSpan={allSectionsCount} className="text-center font-bold text-lg text-dark dark:text-white bg-muted border-t border-b border-gray-200">
                                                {specialEvent.title}
                                            </TableCell>
                                        </TableRow>
                                    );
                                                    ...
                                                    {classesList.length > 0 ? (
                                                        <div className="flex flex-col space-y-2">
                                                            {classesList.map(cls => {
                                                                ...
                                                                return (
                                                                    <div key={`${cls.subject_code}-${cls.teacher_name}`} className={`p-2 rounded-lg ${bg} ${text} shadow-sm transition-transform transform hover:scale-105 min-h-[96px] flex flex-col justify-center items-center ${Object.keys(grade12Data.timetablesByStrand).length > 0 && 'h-[150px]'}`}>
                                                                        <p className="font-bold leading-tight break-words whitespace-normal text-center ">{cls.subject_title} ({cls.subject_code})</p>
                                                                        <p className={`text-2xs mt-1`}>When: {Array.from(cls.days).map(getDayAbbreviation).join(', ')}</p>
                                                                        <p className={`text-2xs leading-tight break-words whitespace-normal text-center`}>Teacher: {cls.teacher_name}</p>
                                                                    </div>
                                                                );
                                                            })}
                                                        </div>
                                                    ...
                                                    {classesList.length > 0 ? (
                                                        <div className="flex flex-col space-y-2">
                                                            {classesList.map(cls => {
                                                                const { bg, text } = subjectColors[cls.subject_code] || { bg: 'bg-gray-100', text: 'text-gray-800' };
                                                                return (
                                                                    <div key={`${cls.subject_code}-${cls.teacher_name}`} className={`p-2 rounded-lg ${bg} ${text} shadow-sm transition-transform transform hover:scale-105  min-h-[96px] flex flex-col justify-center items-center ${Object.keys(grade11Data.timetablesByStrand).length > 0 && 'h-[150px]'}`}>
                                                                        <p className="font-bold leading-tight break-words whitespace-normal text-center">{cls.subject_title} ({cls.subject_code})</p>
                                                                        <p className={`text-2xs mt-1`}>When: {Array.from(cls.days).map(getDayAbbreviation).join(', ')}</p>
                                                                        <p className={`text-2xs  leading-tight break-words whitespace-normal text-center`}>Teacher: {cls.teacher_name}</p>
                                                                    </div>
