...
function JuniorSchedPage() {
  ...
  // Show loading state while data is being fetched
   if ((!trackIsLoading && allTrackData && allTrackData.length > 0 ) || !sectionsIsLoading || !strandIsLoading) {
    return (
       <div className="min-h-screen w-full bg-black relative overflow-hidden ">
      {Object.keys(selectedGeneratedSchedule).length === 0 && selectedGeneratedSchedule && 
        ...
        (
          ...
          />
        <div className="absolute top-[50%] left-[50%] z-3 flex flex-col items-center justify-center transform -translate-x-1/2 -translate-y-1/2">
        <Lock className='h-6 w-6 text-muted-foreground' />
        <h1 className='font-medium text-muted-foreground mt-2'>No schedule selected. Please choose one to proceed.</h1>
      </div>
      </div>
        )
      }

<div className={`container mx-auto p-4 relative ${selectedGeneratedSchedule && Object.keys(selectedGeneratedSchedule).length === 0 ? 'opacity-40' : ''}`}>
